...
def test_read_accounts_seeded(session):
...
def test_update_nonexistent_account(session):
    result = crud.update_account(9999, "Should Not Exist", session)
    assert result == 0

def test_delete_existing_account(session):
...
def test_delete_nonexistent_account(session):
    result = crud.delete_account(9999, session)
    assert result == 0

def test_delete_account_with_recurring_transactions(session):
